...
const FWC = [
  ...
  { id:"FWC2",  name:"Mascote Maple",        foil:true },
  { id:"FWC3",  name:"Mascote Zayu",         foil:true },
  { id:"FWC4",  name:"Mascote Clutch",       foil:true },
  { id:"FWC5",  name:"Slogan Oficial",       foil:true },
  { id:"FWC6",  name:"Bola Trionda",         foil:true },
  { id:"FWC7",  name:"Emblema Canadá",       foil:true },
  { id:"FWC8",  name:"Emblema México",       foil:true },
  { id:"FWC9",  name:"Emblema EUA",          foil:true },
  { id:"FWC10", name:"Momento Copa 1930",    foil:true },
  { id:"FWC11", name:"Momento Copa 1958",    foil:true },
  { id:"FWC12", name:"Momento Copa 1966",    foil:true },
  { id:"FWC13", name:"Momento Copa 1970",    foil:true },
  { id:"FWC14", name:"Momento Copa 1986",    foil:true },
  { id:"FWC15", name:"Momento Copa 1994",    foil:true },
  { id:"FWC16", name:"Momento Copa 1998",    foil:true },
  { id:"FWC17", name:"Momento Copa 2002",    foil:true },
  { id:"FWC18", name:"Momento Copa 2018",    foil:true },
  { id:"FWC19", name:"Momento Copa 2022",    foil:true },
...
const TEAMS = [
  ...
  { code:"SUI", name:"Suíça",              flag:"🇨🇭", color:"#FF0000" },
  // Grupo C
  { code:"BRA", name:"Brasil",             flag:"🇧🇷", color:"#009C3B" },
  { code:"SCO", name:"Escócia",            flag:"🏴󠁧󠁢󠁳󠁣󠁴󠁿", color:"#003DA5" },
  ...
  // Grupo E
  { code:"GER", name:"Alemanha",           flag:"🇩🇪", color:"#000000" },
  { code:"CIV", name:"Costa do Marfim",    flag:"🇨🇮", color:"#F77F00" },
  { code:"CUW", name:"Curaçao",            flag:"🇨🇼", color:"#002B7F" },
  { code:"ECU", name:"Equador",            flag:"🇪🇨", color:"#FFD100" },
  // Grupo F
  ...
  // Grupo L
  { code:"CRO", name:"Croácia",            flag:"🇭🇷", color:"#FF0000" },
  { code:"GHA", name:"Gana",               flag:"🇬🇭", color:"#006B3F" },
  { code:"ENG", name:"Inglaterra",         flag:"🏴󠁧󠁢󠁥󠁮󠁧󠁿", color:"#CF142B" },
  { code:"PAN", name:"Panamá",             flag:"🇵🇦", color:"#005293" },
];

const PLAYER_DATA = {
  MEX:["Team Logo★","Luis Malagón","Johan Vásquez","Jorge Sánchez","César Montes","Jesús Gallardo","Israel Reyes","Diego Lainez","Carlos Rodríguez","Edson Álvarez","Orbelín Pineda","Foto Time","Marcel Ruiz","Érick Sánchez","Hirving Lozano","Santiago Giménez","Raúl Jiménez","Alexis Vega","Roberto Alvarado","César Huerta"],
  RSA:["Team Logo★","Ronwen Williams","Sipho Chaine","Aubrey Modiba","Samukele Kabini","Mbekezeli Mbokazi","Khulumani Ndamane","Siyabonga Ngezana","Khuliso Mudau","Nkosinathi Sibisi","Teboho Mokoena","Foto Time","Thalente Mbatha","Bathuisi Aubaas","Yaya Sithole","Sipho Mbule","Lyle Foster","Ioraam Rayners","Mohau Nkota","Oswin Appolis"],
  KOR:["Team Logo★","Hyeon-woo Jo","Seung-Gyu Kim","Min-jae Kim","Yu-min Cho","Young-woo Seol","Han-beom Lee","Tae-seok Lee","Myung-jae Lee","Jae-sung Lee","In-beom Hwang","Foto Time","Kang-in Lee","Seung-ho Paik","Jens Castrop","Dong-gyeong Lee","Gue-sung Cho","Heung-min Son","Hee-chan Hwang","Hyeon-Gyu Oh"],
  CZE:["Team Logo★","Matěj Kovář","Jindřich Staněk","Ladislav Krejčí","Vladimír Coufal","Jaroslav Zelený","Tomáš Holeš","David Zima","Michal Sadílek","Lukáš Provod","Lukáš Červ","Foto Time","Tomáš Souček","Pavel Šulc","Matěj Vydra","Vasil Kušej","Tomáš Chorý","Václav Černý","Adam Hložek","Patrik Schick"],
  BIH:["Team Logo★","Nikola Vasilj","Amar Dedić","Sead Kolašinac","Tarik Muharemović","Nihad Mujakić","Nikola Katić","Amir Hadžiahmetović","Benjamin Tahirović","Armin Gigović","Ivan Šunjić","Foto Time","Ivan Bašić","Dženis Burnić","Esmir Bajraktarević","Amar Memić","Ermedin Demirović","Edin Džeko","Samed Baždar","Haris Tabaković"],
  CAN:["Team Logo★","Dayne St. Clair","Alphonso Davies","Alistair Johnston","Samuel Adekugbe","Richie Laryea","Derek Cornelius","Moïse Bombito","Kamal Miller","Stephen Eustáquio","Ismaël Koné","Foto Time","Jonathan Osorio","Jacob Shaffelburg","Mathieu Choinière","Niko Sigur","Tajon Buchanan","Liam Millar","Cyle Larin","Jonathan David"],
  QAT:["Team Logo★","Meshaal Barsham","Sultan Albrake","Lucas Mendes","Homam Ahmed","Boualem Khoukhi","Pedro Miguel","Tarek Salman","Mohamed Al-Mannai","Karim Boudiaf","Assim Madibo","Foto Time","Ahmed Fatehi","Mohammed Waad","Abdulaziz Hatem","Hassan Al-Haydos","Edmilson Junior","Akram Hassan Afif","Ahmed Al Ganehi","Almoez Ali"],
  SUI:["Team Logo★","Gregor Kobel","Yvon Mvogo","Manuel Akanji","Ricardo Rodriguez","Nico Elvedi","Aurèle Amenda","Silvan Widmer","Granit Xhaka","Denis Zakaria","Remo Freuler","Foto Time","Fabian Rieder","Ardon Jashari","Johan Manzambi","Michel Aebischer","Breel Embolo","Ruben Vargas","Dan Ndoye","Zeki Amdouni"],
  BRA:["Team Logo★","Alisson","Bento","Marquinhos","Éder Militão","Gabriel Magalhães","Danilo","Wesley","Lucas Paquetá","Casemiro","Bruno Guimarães","Foto Time","Luiz Henrique","Vinícius Júnior","Rodrygo","João Pedro","Matheus Cunha","Gabriel Martinelli","Raphinha","Estêvão"],
  SCO:["Team Logo★","Angus Gunn","Jack Hendry","Kieran Tierney","Aaron Hickey","Andrew Robertson","Scott McKenna","John Souttar","Anthony Ralston","Grant Hanley","Scott McTominay","Foto Time","Billy Gilmour","Lewis Ferguson","Ryan Christie","Kenny McLean","John McGinn","Lyndon Dykes","Che Adams","Ben Doak"],
  HAI:["Team Logo★","Johny Placide","Carlens Arcus","Martin Expérience","Jean-Kevin Duverne","Ricardo Adé","Duke Lacroix","Garven Metusala","Hannes Delcroix","Leverton Pierre","Danley Jean Jacques","Foto Time","Jean-Ricner Bellegarde","Christopher Attys","Derrick Etienne Jr.","Josué Casimir","Ruben Providence","Duckens Nazon","Louicius Deedson","Frantzdy Pierrot"],
  MAR:["Team Logo★","Yassine Bounou","Munir El Kajoui","Achraf Hakimi","Noussair Mazraoui","Nayef Aguerd","Romain Saïss","Jawad El Yamiq","Adam Masina","Sofyan Amrabat","Azzedine Ounahi","Foto Time","Eliesse Ben Seghir","Bilal El Khannouss","Ismael Saibari","Youssef En-Nesyri","Abde Ezzalzouli","Soufiane Rahimi","Brahim Díaz","Ayoub El Kaabi"],
  AUS:["Team Logo★","Mathew Ryan","Joe Gauci","Harry Souttar","Alessandro Circati","Jordan Bos","Aziz Behich","Cameron Burgess","Lewis Miller","Milos Degenek","Jackson Irvine","Foto Time","Riley McGree","Aiden O'Neill","Connor Metcalfe","Patrick Yazbek","Craig Goodwin","Kusini Yengi","Nestory Irankunda","Mohamed Touré"],
  USA:["Team Logo★","Matt Freese","Chris Richards","Tim Ream","Mark McKenzie","Alex Freeman","Antonee Robinson","Tyler Adams","Tanner Tessmann","Weston McKennie","Christian Roldan","Foto Time","Timothy Weah","Diego Luna","Malik Tillman","Christian Pulisic","Brenden Aaronson","Ricardo Pepi","Haji Wright","Folarin Balogun"],
  PAR:["Team Logo★","Roberto Fernández","Orlando Gill","Gustavo Gómez","Fabián Balbuena","Juan José Cáceres","Omar Alderete","Junior Alonso","Mathías Villasanti","Diego Gómez","Damián Bobadilla","Foto Time","Andrés Cubas","Matías Galarza","Richard Sánchez","Julio Enciso","Robert Morales","Miguel Almirón","Sergio Díaz","Ramón Sosa"],
  TUR:["Team Logo★","Ugurcan Cakir","Mert Muldur","Zeki Celik","Abdulkerim Bardakci","Caglar Soyuncu","Merih Demiral","Ferdi Kadioglu","Kaan Ayhan","Ismail Yuksek","Hakan Calhanoglu","Foto Time","Orkun Kokcu","Arda Güler","Irfan Can Kahveci","Yunus Akgun","Can Uzun","Baris Alper Yilmaz","Kerem Akturkoglu","Kenan Yildiz"],
  GER:["Team Logo★","Marc-André ter Stegen","Jonathan Tah","David Raum","Nico Schlotterbeck","Antonio Rüdiger","Waldemar Anton","Ridle Baku","Maximilian Mittelstädt","Joshua Kimmich","Florian Wirtz","Foto Time","Felix Nmecha","Leon Goretzka","Jamal Musiala","Serge Gnabry","Kai Havertz","Leroy Sané","Karim Adeyemi","Nick Woltemade"],
  CIV:["Team Logo★","Yahia Fofana","Ghislain Konan","Wilfried Singo","Odilon Kossounou","Evan Ndicka","Willy Boly","Emmanuel Agbadou","Ousmane Diomande","Franck Kessié","Seko Fofana","Foto Time","Ibrahim Sangaré","Jean-Philippe Gbamin","Amad Diallo","Sébastien Haller","Simon Adingra","Yan Diomande","Evann Guessand","Oumar Diakité"],
  CUW:["Team Logo★","Eloy Room","Armando Obispo","Sherel Floranus","Jurien Gaari","Joshua Brenet","Roshon Van Eijma","Shurandy Sambo","Livano Comenencia","Godfried Roemeratoe","Juninho Bacuna","Foto Time","Leandro Bacuna","Tahith Chong","Kenji Gorré","Jearl Margaritha","Jurgen Locadia","Jeremy Antonisse","Gervane Kastaneer","Sontje Hansen"],
  ECU:["Team Logo★","Hernán Galíndez","Gonzalo Valle","Piero Hincapié","Pervis Estupiñán","Willian Pacho","Ángelo Preciado","Joel Ordóñez","Moisés Caicedo","Alan Franco","Kendry Páez","Foto Time","Pedro Vite","John Yeboah","Leonardo Campana","Gonzalo Plata","Nilson Angulo","Alan Minda","Kevin Rodríguez","Enner Valencia"],
  NED:["Team Logo★","Bart Verbruggen","Virgil van Dijk","Micky van de Ven","Jurriën Timber","Denzel Dumfries","Nathan Aké","Jeremie Frimpong","Jan Paul van Hecke","Tijjani Reijnders","Ryan Gravenberch","Foto Time","Teun Koopmeiners","Frenkie de Jong","Xavi Simons","Justin Kluivert","Memphis Depay","Donyell Malen","Wout Weghorst","Cody Gakpo"],
  JPN:["Team Logo★","Zion Suzuki","Henry Heroki Mochizuki","Ayumu Seko","Junnosuke Suzuki","Shogo Taniguchi","Tsuyoshi Watanabe","Kaishu Sano","Yuki Soma","Ao Tanaka","Daichi Kamada","Foto Time","Takefusa Kubo","Ritsu Doan","Keito Nakamura","Takumi Minamino","Shuto Machino","Junya Ito","Koki Ogawa","Ayase Ueda"],
  SWE:["Team Logo★","Victor Johansson","Isak Hien","Gabriel Gudmundsson","Emil Holm","Victor Nilsson Lindelöf","Gustaf Lagerbielke","Lucas Bergvall","Hugo Larsson","Jesper Karlström","Yasin Ayari","Foto Time","Mattias Svanberg","Daniel Svensson","Ken Sema","Roony Bardghji","Dejan Kulusevski","Anthony Elanga","Alexander Isak","Viktor Gyökeres"],
  TUN:["Team Logo★","Bechir Ben Said","Aymen Dahmen","Van Valery","Montassar Talbi","Yassine Meriah","Ali Abdi","Dylan Bronn","Ellyes Skhiri","Aissa Laidouni","Ferjani Sassi","Foto Time","Mohamed Ali Ben Romdhane","Hannibal Mejbri","Elias Achouri","Elias Saad","Hazem Mastouri","Ismael Gharbi","Sayfallah Ltaief","Naim Sliti"],
  BEL:["Team Logo★","Thibaut Courtois","Arthur Theate","Timothy Castagne","Zeno Debast","Brandon Mechele","Maxim De Cuyper","Thomas Meunier","Youri Tielemans","Amadou Onana","Nicolas Raskin","Foto Time","Alexis Saelemaekers","Hans Vanaken","Kevin De Bruyne","Jérémy Doku","Charles De Ketelaere","Leandro Trossard","Loïs Openda","Romelu Lukaku"],
  EGY:["Team Logo★","Mohamed El Shenawy","Mohamed Hany","Mohamed Hamdy","Yasser Ibrahim","Khaled Sobhi","Ramy Rabia","Hossam Abdelmaguid","Ahmed Fatouh","Marwan Attia","Zizo","Foto Time","Hamdy Fathy","Mohamed Lasheen","Emam Ashour","Osama Faisal","Mohamed Salah","Mostafa Mohamed","Trezeguet","Omar Marmoush"],
  IRN:["Team Logo★","Alireza Beiranvand","Morteza Pouraliganji","Ehsan Hajsafi","Milad Mohammadi","Shoja Khalilzadeh","Ramin Rezaeian","Hossein Kanaani","Sadegh Moharrami","Saleh Hardani","Saeed Ezatolahi","Foto Time","Saman Ghoddos","Omid Noorafkan","Roozbeh Cheshmi","Mohammad Mohebi","Sardar Azmoun","Mehdi Taremi","Alireza Jahanbakhsh","Ali Gholizadeh"],
  NZL:["Team Logo★","Max Crocombe-Payne","Alex Paulsen","Michael Boxall","Liberato Cacace","Tim Payne","Tyler Bindon","Francis de Vries","Finn Surman","Joe Bell","Sarpreet Singh","Foto Time","Ryan Thomas","Matthew Garbett","Marko Stamenić","Ben Old","Chris Wood","Elijah Just","Callum McCowatt","Kosta Barbarouses"],
  KSA:["Team Logo★","Nawaf Alaqidi","Abdulrahman Al-Sanbi","Saud Abdulhamid","Nawaf Boushal","Jihad Thakri","Moteb Al-Harbi","Hassan Altambakti","Musab Aljuwayr","Ziyad Aljohani","Abdullah Alkhaibari","Foto Time","Nasser Aldawsari","Saleh Abu Alshamat","Marwan Alsahafi","Salem Aldawsari","Abdulrahman Al-Aboud","Feras Albrikan","Saleh Alshehri","Abdullah Al-Hamdan"],
  CPV:["Team Logo★","Vozinha","Logan Costa","Pico","Diney","Steven Moreira","Wagner Pina","João Paulo","Yannick Semedo","Kevin Pina","Patrick Andrade","Foto Time","Jamiro Monteiro","Deroy Duarte","Garry Rodrigues","Jovane Cabral","Ryan Mendes","Dailon Livramento","Willy Semedo","Bebé"],
  ESP:["Team Logo★","Unai Simón","Robin Le Normand","Aymeric Laporte","Dean Huijsen","Pedro Porro","Dani Carvajal","Marc Cucurella","Martín Zubimendi","Rodri","Pedri","Foto Time","Fabián Ruiz","Mikel Merino","Lamine Yamal","Dani Olmo","Nico Williams","Ferran Torres","Álvaro Morata","Mikel Oyarzabal"],
  URU:["Team Logo★","Sergio Rochet","Santiago Mele","Ronald Araujo","José María Giménez","Sebastian Caceres","Mathias Olivera","Guillermo Varela","Nahitan Nandez","Federico Valverde","Giorgian De Arrascaeta","Foto Time","Rodrigo Bentancur","Manuel Ugarte","Nicolás de la Cruz","Maxi Araujo","Darwin Núñez","Federico Viñas","Rodrigo Aguirre","Facundo Pellistri"],
  FRA:["Team Logo★","Mike Maignan","Theo Hernández","William Saliba","Jules Koundé","Ibrahima Konaté","Dayot Upamecano","Lucas Digne","Aurélien Tchouaméni","Eduardo Camavinga","Manu Koné","Foto Time","Adrien Rabiot","Michael Olise","Ousmane Dembélé","Bradley Barcola","Désiré Doué","Kingsley Coman","Hugo Ekitike","Kylian Mbappé"],
  IRQ:["Team Logo★","Jalal Hassan","Rebin Sulaka","Hussein Ali","Akam Hashem","Merchas Doski","Zaid Tahseen","Manaf Younis","Zidane Iqbal","Amir Al-Ammari","Ibrahim Bayesh","Foto Time","Ali Jasim","Youssef Amyn","Aimar Sher","Marko Farji","Osama Rashid","Ali Al-Hamadi","Aymen Hussein","Mohanad Ali"],
  NOR:["Team Logo★","Ørjan Nyland","Julian Ryerson","Leo Østigård","Kristoffer Ajer","Marcus Holmgren Pedersen","David Møller Wolfe","Torbjørn Heggem","Morten Thorsby","Martin Ødegaard","Sander Berge","Foto Time","Andreas Schjelderup","Patrick Berg","Erling Haaland","Alexander Sørloth","Aron Dønnum","Jørgen Strand Larsen","Antonio Nusa","Oscar Bobb"],
  SEN:["Team Logo★","Eduardo Mendy","Yehvann Diouf","Moussa Niakhaté","Abdoulaye Seck","Ismail Jakobs","El Hadji Malick Diouf","Kalidou Koulibaly","Idrissa Gana Gueye","Pape Matar Sarr","Pape Gueye","Foto Time","Habib Diarra","Lamine Camara","Sadio Mane","Ismaïla Sarr","Boulaye Dia","Iliman Ndiaye","Nicolas Jackson","Krepin Diatta"],
  ALG:["Team Logo★","Alexis Guendouz","Ramy Bensebaini","Youcef Atal","Riyad Mahrez","Islam Slimani","Andy Delort","Haris Belkebla","Houssem Aouar","Sofiane Feghouli","Aïssa Mandi","Foto Time","Ismaël Bennacer","Mehdi Zerkane","Yacine Brahimi","Adam Ounas","Baghdad Bounedjah","Loïs Diony","Ilyes Chetti","Farès Chaïbi"],
  ARG:["Team Logo★","Emiliano Martínez","Nahuel Molina","Cristian Romero","Nicolás Otamendi","Nicolás Tagliafico","Leonardo Balerdi","Enzo Fernández","Alexis Mac Allister","Rodrigo De Paul","Exequiel Palacios","Foto Time","Leandro Paredes","Nico Paz","Franco Mastantuono","Nico González","Lionel Messi","Lautaro Martínez","Julián Álvarez","Giuliano Simeone"],
  AUT:["Team Logo★","Patrick Pentz","Alexander Schlager","Philipp Lienhart","Stefan Posch","David Alaba","Maximilian Wöber","Phillipp Mwene","Christoph Baumgartner","Konrad Laimer","Marcel Sabitzer","Foto Time","Florian Grillitsch","Nicolas Seiwald","Xaver Schlager","Marko Arnautovic","Patrick Wimmer","Michael Gregoritsch","Romano Schmid","Louis Schaub"],
  JOR:["Team Logo★","Yazeed Abo Laila","Ahmad Alhajali","Yazan Alanbi","Baha Abdelrahman","Ahmad Qatawneh","Omar Al-Dardour","Musa Al-Taamari","Osama Rashdan","Mothanna Zaid","Abdullah Nasib","Foto Time","Ali Olwan","Nour Mansour","Hamza Al-Dardour","Mohammad Abu Zreiq","Mousa Suleiman","Ibrahim Saleh","Yahia Naser","Ahmad Hayel"],
  COL:["Team Logo★","Camilo Vargas","David Ospina","Dávinson Sánchez","Yerry Mina","William Tesillo","Johan Mojica","Daniel Muñoz","Wilmar Barrios","Juan Cuadrado","James Rodríguez","Foto Time","Mateus Uribe","Jhon Arias","Luis Sinisterra","Rafael Santos Borré","Falcao García","Duván Zapata","Luis Díaz","Jhon Jáder Durán"],
  POR:["Team Logo★","Diogo Costa","Rui Patrício","Rúben Dias","Pepe","Danilo Pereira","Nuno Mendes","João Cancelo","Vitinha","Bruno Fernandes","Bernardo Silva","Foto Time","Rúben Neves","João Palhinha","Rafael Leão","Diogo Jota","André Silva","Pedro Neto","Gonçalo Ramos","Cristiano Ronaldo"],
  COD:["Team Logo★","Joël Kiassumbua","Leysen Bula","Chancel Mbemba","Wilfried Kanoko","Arthur Masuaku","Yannick Bolasie","Merveille Bokadi","Cédric Bakambu","Dieumerci Mbokani","Theo Bongonda","Foto Time","Yannick Ferreira-Carrasco","Silas Wissa","Jonathan Bolingi","Britt Assombalonga","Christian Luyindama","Neeskens Kebano","Silas","Mechak Elia"],
  UZB:["Team Logo★","Otabek Shukurov","Dostonbek Khamdamov","Dilshod Narzullayev","Jasur Yakhshiboev","Shamsiddin Mirzo","Jaloliddin Masharipov","Eldor Shomurodov","Otabek Suyunov","Azizbek Turgunboev","Abbosbek Fayzullayev","Foto Time","Khojiakbar Alijonov","Khamza Kamalov","Nodir Tursunov","Sanjar Tursunov","Temur Musaev","Bahodir Suyunov","Mirzo Akhmedov","Jasurbek Yakhshiboev"],
  CRO:["Team Logo★","Dominik Livaković","Ivica Ivušić","Joško Gvardiol","Dejan Lovren","Borna Sosa","Josip Šutalo","Domagoj Vida","Ivan Perišić","Luka Modrić","Mateo Kovačić","Foto Time","Marcelo Brozović","Mario Pašalić","Andrej Kramarić","Bruno Petković","Marko Livaja","Ante Budimir","Nikola Vlašić","Petar Sučić"],
  GHA:["Team Logo★","Lawrence Ati-Zigi","Ibrahim Danlad","Alexander Djiku","Daniel Amartey","Abdul Rahman Baba","Tariq Lamptey","Jonathan Mensah","Thomas Partey","Mohammed Kudus","Jordan Ayew","Foto Time","André Ayew","Felix Afena-Gyan","Osman Bukari","Inaki Williams","Antoine Semenyo","Kamaldeen Sulemana","Joel Fameyeh","Ernest Nuamah"],
  ENG:["Team Logo★","Jordan Pickford","Aaron Ramsdale","Harry Maguire","John Stones","Luke Shaw","Kyle Walker","Kieran Trippier","Declan Rice","Jude Bellingham","Phil Foden","Foto Time","Marcus Rashford","Bukayo Saka","Raheem Sterling","Jack Grealish","Harry Kane","Ollie Watkins","Cole Palmer","Kobbie Mainoo"],
  PAN:["Team Logo★","Luis Mejía","José Calderón","Harold Cummings","Fidel Escobar","Andrés Andrade","Edgardo Fariña","Adalberto Carrasquilla","Álex Rodríguez","Aníbal Godoy","Gabriel Torres","Foto Time","Adolfo Machado","Freddy Góndola","Édgar Bárcenas","Rolando Blackburn","Ismael Díaz","Michael Murillo","Jorman Aguilar","Cecilio Waterman"],
};
